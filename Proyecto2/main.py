...
def principal():
    ...
    if cadena == 'hola':
        txt.config(state='normal')
        txt.insert("end", 'que onda bro\n', ("LEFT",))
        txt.tag_configure("left", justify="left")
        txt.config(state='disabled')
        txt.config(state='normal')
        txt.insert("end", 'adios', ("LEFT",))
...
if __name__ == '__main__':
    ...
    btn = Button(window, text="ENVIAR",bg='SeaGreen',borderwidth=4,width=16,fg='white',font='Arial 10 bold',command=principal,height=1)
